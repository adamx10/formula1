...
export function CarSlider() {
  ...
  return (
    ...
    >
      ...
      {!isMobile && (
        <>
          <div className="absolute top-0 left-0 z-10 h-full w-48 bg-gradient-to-r from-carbon-950 to-transparent pointer-events-none" />
          <div className="absolute top-0 right-0 z-10 h-full w-48 bg-gradient-to-l from-carbon-950 to-transparent pointer-events-none" />
        </>
      )}
      ...
      >
        {CAR_IMAGES.map((src, i) => (
          <div 
            key={i} 
            className="group relative flex-shrink-0 w-[300px] sm:w-[500px] aspect-video overflow-hidden rounded-sm border border-carbon-500/50 transition-all duration-500 hover:border-blue-500/50 hover:shadow-2xl hover:shadow-blue-500/20 snap-center"
          >
            <img
              src={src}
              alt={`F1 Car ${i + 1}`}
            ...
            />
            {/* Glossy overlay */}
            <div className="absolute inset-0 bg-gradient-to-t from-carbon-900/90 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
