          ...
          <div className="flex rounded-sm border border-carbon-500 overflow-hidden">
            {(['drivers', 'constructors'] as Tab[]).map((t) => (
              <button key={t} onClick={() => setTab(t)}
                className={`px-4 py-2 text-xs font-bold uppercase tracking-widest transition-colors ${tab === t ? 'bg-red-500 text-white shadow-[0_0_15px_rgba(225,6,0,0.4)]' : 'bg-carbon-800 text-carbon-100 hover:text-white'}`}>
                {t}
              </button>
            ))}
          </div>
          ...
          {sortedDrivers.map((d, i) => {
            ...
            return (
              <Link key={d.id} to={`/drivers/${d.id}`}
                className="stand-row flex items-center gap-3 rounded-sm border border-carbon-500 bg-carbon-800 px-4 py-3 hover:border-red-500/40 hover:bg-carbon-700 transition-all group">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-1.5 h-0.5 w-full rounded-full bg-carbon-600">
                    <div className="stand-bar h-full rounded-full origin-left" style={{ width: `${pct}%`, backgroundColor: team?.primaryColor ?? '#e10600' }} />
                  </div>
                </div>
          ...
          {sortedTeams.map((t, i) => {
            ...
            return (
              <Link key={t.id} to={`/teams/${t.id}`}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-white group-hover:text-red-400 transition-colors">{t.name}</span>
                  </div>
